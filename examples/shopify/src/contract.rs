...
impl NonFungibleTokenContract {
    ...
    async fn transfer(
        ...
        if target_account.chain_id == self.runtime.chain_id() {
            ...
            self.state
                .balances
                .for_each_index_value(|_token, _amount| {
                    if _token == token && !found {
                        let curr_amt = _amount
                            .is_empty()
                            .then(|| "0".to_string())
                            .unwrap_or(_amount.to_string());
                        let mut curr_amt_f64: f64 = curr_amt.parse().unwrap();
                        curr_amt_f64 += amount.parse::<f64>().unwrap();
            ...
            if !found {
                updates.push((token.clone(), amount.clone()));
            }
    ...
    async fn withdraw(&mut self, token: String, amount: String) {
        ...
        self.state
            ...
            .for_each_index_value(|_token, _amount| {
                if _token == token {
                    let __amount = amount.parse::<f64>().unwrap();
                    let curr_amt = _amount
                        .is_empty()
                        .then(|| "0".to_string())
                        .unwrap_or(_amount.to_string());
