...
impl WithServiceAbi for NonFungibleTokenService {
    type Abi = shopify::NonFungibleTokenAbi;
}
...
impl QueryRoot {
    ...
    async fn filter_nfts(&self, nft_type: NftStatus) -> BTreeMap<String, NftOutput> {
        let mut nfts = BTreeMap::new();
        self.non_fungible_token
            .nfts
            .for_each_index_value(|_token_id, nft| {
    ...
    async fn balances(&self) -> BTreeMap<String, String> {
        let mut balances = BTreeMap::new();
        self.non_fungible_token
            .balances
            .for_each_index_value(|_token, amount| {
                balances.insert(_token, amount.into_owned());
                Ok(())
            })
            .await
            .unwrap();
